...
app.get('/login', async (req, res) => {
  const parentSpan = tracer.startSpan('web2.login');
...
app.get('/trace', async (req, res) => {
  ...
  const childSpan = tracer.startSpan('web2.trace', {
...
app.get('/logout', async (req, res) => {
  ...
  // 親スパンのコンテキストを使用して新しいスパンを作成
  const logoutSpan = tracer.startSpan('web2.logout', {
    childOf: parentSpanContext
  });
  ...
  setTimeout(() => {
    ...
    // 親スパンを終了
    const parentSpan = tracer.startSpan('web2.parent_finish', {
      childOf: parentSpanContext
    });
    parentSpan.finish();

    // Redisからスパン情報を削除 エラーになるのでコメントアウト
